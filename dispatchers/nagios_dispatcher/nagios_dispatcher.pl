...
TEMPORARY NOTE DURING DEV:
Here is what is needed in the schema to make it work:
create table hub (batch_nr bigint,data text[]);
create sequence hub_sequence;
...
sub insert_parsed_data
{
	...
	# get a batch number
	my $ref_idbatch = $dbh->selectall_arrayref("SELECT nextval('hub_sequence')") or return 0; #Failure
	my $batch_number=$ref_idbatch->[0]->[0];
	my $sth=$dbh->do('COPY hub FROM STDIN');
	foreach my $record (@{$parsed_data})
	{
		...
		unless ($executed)
		{
			log_message "Can't COPY this record: $record->{HOSTNAME},$record->{TIMET},$record->{SERVICEDESC},$record->{SERVICESTATE},$record->{LABEL},$record->{VALUE},$record->{MIN},$record->{MAX},$record->{WARNING},$record->{CRITICAL},$record->{UOM}.\nFile : $filename \n";
			return 0; #Failure
		}
	}
	$dbh->pg_putcopyend();
