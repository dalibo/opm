...
sub startup {
    ...
    $self->plugins->namespaces([ "Helpers", "PgFactory", @{ $self->plugins->namespaces } ]);

    # setup charset
    $self->plugin( charset => { charset => 'utf8' } );
    $self->plugin( I18N => {namespace => 'PgFactory::I18N', support_url_langs => [qw(en fr)]} );

    # load configuration
    my $config_file = $self->home.'/pgfactory.conf';
    ...
    $r_adm->route('/user/delete/:rolname')              ->to('user#delete')       ->name('user_delete');
    $r_adm->route('/user/delacc/:accname/:rolname')     ->to('user#delacc')       ->name('user_delacc');

    # Account management
    $r_adm->route('/account')                           ->to('account#list')      ->name('account_list');
    $r_adm->route('/account/create')                    ->to('account#create')    ->name('account_create');
    $r_adm->route('/account/:accname')                  ->to('account#edit')      ->name('account_edit');
    $r_adm->route('/account/delete/:accname')           ->to('account#delete')    ->name('account_delete');
    $r_adm->route('/account/delrol/:accname/:rolname')  ->to('account#delrol')    ->name('account_delrol');
