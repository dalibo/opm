...
sub host{
    ...
    $sql = $dbh->prepare("SELECT s1.id,s1.warehouse,s1.service,s1.last_modified,s1.creation_ts,s1.servalid, g.id, g.graph FROM public.list_services() s1 JOIN public.list_servers() s2 ON s2.id = s1.id_server JOIN pr_grapher.graph_services gs ON gs.id_service = s1.id JOIN pr_grapher.graphs g ON g.id = gs.id_graph WHERE s2.hostname = '$hostname';");
    $sql->execute();
    my $services = [];
