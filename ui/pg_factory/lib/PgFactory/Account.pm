...
sub list{
    ...
    if ($method =~ m/^POST$/i) { # Create a new account
        ...
        if (!$e){
            $sql = $dbh->prepare("SELECT public.create_account('".$form_data->{accname}."');");
            if ($sql->execute()){
...
sub edit{
    ...
    if ($method =~ m/^POST$/i) {
        ...
        if (!$form_data->{existing_username} =~ m/^\s*$/) {
            ...
            $sql = $dbh->prepare('GRANT "'.$accname.'" TO "'.$form_data->{existing_username}.'";');
            if ($sql->execute()){
            ...
            if (!$e){
                $sql = $dbh->prepare("SELECT public.create_user('".$form_data->{username}."','".$form_data->{password}."','{".$accname."}');");
                if ($sql->execute()){
                    $self->msg->info("User added");
                } else{
                    $self->msg->error("Could not add user");
    ...
    $sql = $dbh->prepare("SELECT useid,rolname FROM list_users('$accname') ORDER BY 2;");
    $sql->execute();
    my $roles = [];
    ...
    $sql = $dbh->prepare("SELECT DISTINCT rolname FROM list_users() EXCEPT SELECT rolname FROM list_users('$accname') ORDER BY 1;");
    $sql->execute();
    my $allroles = [];
