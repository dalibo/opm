...
has msg_lists => sub { { debug => [], info => [], error => [], warning => [] } };
...
sub register {
    ...
    $app->helper(
		 display_messages => sub {
			 ...
			 $html .= join("\n", map { "<li>".$ctrl->l($_)."</li>" } @{$self->msg_lists->{debug}});
			 $html .= qq{</ul></div>};
		     }
			 ...
			 $html .= qq{<button type="button" class="close" data-dismiss="alert">&times;</button>\n};
			 $html .= qq{<ul class="unstyled">\n};
			 $html .= join("\n", map { "<li>".$ctrl->l($_)."</li>" } @{$self->msg_lists->{error}});
			 $html .= qq{</ul></div>};
		     }
			 ...
			 $html .= qq{<button type="button" class="close" data-dismiss="alert">&times;</button>\n};
			 $html .= qq{<ul class="unstyled">\n};
			 $html .= join("\n", map { "<li>".$ctrl->l($_)."</li>" } @{$self->msg_lists->{warning}});
			 $html .= qq{</ul></div>};
		     }
			 ...
			 $html .= qq{<button type="button" class="close" data-dismiss="alert">&times;</button>\n};
			 $html .= qq{<ul class="unstyled">\n};
			 $html .= join("\n", map { "<li>".$ctrl->l($_)."</li>" } @{$self->msg_lists->{info}});
			 $html .= qq{</ul></div>};
		     }

		     # Empty the message list so that it is displayed only once
		     $self->msg_lists({ debug => [], info => [], error => [], warning => [] });
