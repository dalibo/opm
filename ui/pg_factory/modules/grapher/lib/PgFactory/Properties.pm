...
sub save {
    ...
    open(FH, "> $file") or croak "Could not save default properties file: $!";;
    print FH $json;
    close(FH);
}
...
sub validate {
    ...
    foreach my $c (qw/xaxis_showLabels yaxis_showLabels y2axis_showLabels bars_stacked
    ...
    # Process numbers
    foreach my $k (qw/points_lineWidth bars_lineWidth y2axis_titleAngle yaxis_labelsAngle points_radius
...
sub to_plot {
	...
	if ($p eq "show_legend") {
	    $options->{legend}->{container} = ($props->{$p}) ? undef : $props->{$p};
	    next;
	}
	...
	if ($p eq 'type') {
	    $options->{$props->{$p}} = { } if ! exists $options->{$props->{$p}};
