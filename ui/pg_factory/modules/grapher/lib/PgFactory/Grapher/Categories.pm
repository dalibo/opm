package PgFactory::Grapher::Categories;
...
sub list {
    ...
    my $sth = $dbh->prepare(qq{SELECT p.id, p.category, p.description, string_agg(c.category, ', ' ORDER BY c.category) AS children
...
sub add {
    ...
    if ($method =~ m/^POST$/i) {
		...
		my $sth = $dbh->prepare(qq{INSERT INTO pr_grapher.categories (category, description) VALUES (?, ?) RETURNING id});
		if (!defined $sth->execute($form->{category},
					   ($form->{description} eq '') ?
					   undef : $form->{category})) {
		...
		# Membership
		$sth = $dbh->prepare(qq{INSERT INTO pr_grapher.nested_categories (id_parent, id_child) VALUES (?, ?)});
		if (exists $form->{parents}) {
		    my %p = map { $_, 1 } (ref $form->{parents} eq '') ? ($form->{parents}) : @{$form->{parents}};
		    foreach my $i (keys %p) {
		...
		if (exists $form->{children}) {
		    my %c = map { $_, 1 } (ref $form->{children} eq '') ? ($form->{children}) : @{$form->{children}};
		    foreach my $i (keys %c) {
...
sub edit {
    ...
    # Get the category
    my $sth = $dbh->prepare(qq{SELECT category, description FROM pr_grapher.categories WHERE id = ?});
    $sth->execute($id);
    ...
    if ($method =~ m/^POST$/i) {
		...
		# Update the category
		$sth = $dbh->prepare(qq{UPDATE pr_grapher.categories SET category = ?, description = ? WHERE id = ?});
		if (!defined $sth->execute($form->{category},
					   ($form->{description} eq '') ? undef : $form->{description},
					   $id)) {
		    $self->render_exception($dbh->errstr);
		    $sth->finish;
		    $dbh->rollback;
		...
		$sth = $dbh->prepare(qq{DELETE FROM pr_grapher.nested_categories WHERE id_parent = ? OR id_child = ?});
    ...
    $sth = $dbh->prepare(qq{SELECT id_parent FROM pr_grapher.nested_categories WHERE id_child = ?});
    $sth->execute($id);
    my $parents = [ ];
    ...
    $sth = $dbh->prepare(qq{SELECT id_child FROM pr_grapher.nested_categories WHERE id_parent = ?});
    $sth->execute($id);
    my $children = [ ];
    while (my ($i) = $sth->fetchrow) {
	push @$children, $i;
...
sub remove {
	...
	# Action depends on the name of the button pressed
	    ...
	    $sth = $dbh->prepare(qq{DELETE FROM pr_grapher.categories WHERE id = ?});
    ...
    $sth = $dbh->prepare(qq{SELECT string_agg(c.category, ', ' ORDER BY c.category)
